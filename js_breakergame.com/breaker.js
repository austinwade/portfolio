...
let field = {
    ...
    drawBlocks: function () {
        ...
        for (let i = 0; i < field.blocks.length; i++) {
            ...
            this.ctx.fillText(
                this.blocks[i][2],
                this.blocks[i][0] + (this.delta - this.space) / 2,
                this.blocks[i][1] + (this.delta - this.space) * 0.65
            );
        }
    ...
    genRow: function () {
        for (
            let i = 0;
            i <
            Math.floor((this.canvas.width - this.constraint) / this.delta) - 1;
            i++
        ) {
            if (Math.random() > 0.5) {
                this.blocks.push([
                    i * this.delta + this.constraint,
                    -field.delta,
                    Math.ceil((this.level + 1) * 2 * Math.random()),
                ]);
            } else if (Math.random() > 0.75)
                this.newBalls.push([
                    i * this.delta + this.constraint + field.delta / 2,
                    -field.delta / 2,
                ]);
        }
    },
    checkNewBallTouches: function (balls) {
        for (let j = 0; j < balls.length; j++) {
            ...
            for (let i = 0; i < this.newBalls.length; i++) {
                if (
                    Math.sqrt(
                        Math.pow(this.newBalls[i][0] - x, 2) +
                            Math.pow(this.newBalls[i][1] - y, 2)
                    ) <=
                    myBalls.radius + this.newBallsRadius
                ) {
                    myBalls.numBalls++;
                    document.getElementsByClassName("ballnum")[0].innerHTML =
                        myBalls.numBalls;
    ...
    checkCollisions: function (balls) {
        ...
        for (let j = 0; j < balls.length; j++) {
            ...
            for (let i = 0; i < this.blocks.length; i++) {
                if (
                    x >= this.blocks[i][0] - myBalls.radius &&
                    x <=
                        this.blocks[i][0] +
                            field.delta +
                            myBalls.radius -
                    ...
                    if (
                        y >= this.blocks[i][1] - myBalls.radius &&
                        y <=
                            this.blocks[i][1] +
                                field.delta +
                                myBalls.radius -
...
function balls() {
    ...
    (this.advanceBalls = function () {
        for (let i = 0; i < this.balls.length; i++) {
            ...
            } else if (this.balls[i][0] <= this.radius)
                this.balls[i][2] = Math.abs(this.balls[i][2]);
        ...
        (this.addBall = function () {
            field.startClock();
            this.balls.push([
                field.canvas.width / 2,
                field.canvas.height - myBalls.radius,
                this.speedSet[0],
                -this.speedSet[1],
...
document.onmousemove = function (e) {
    if (field.interval == null) {
        ...
        field.ctx.lineTo(
            e.clientX - (window.innerWidth - field.canvas.width) / 2,
            e.clientY
        );
        field.ctx.stroke();
...
document.onkeydown = function (e) {
    ...
    if ([32, 37, 38, 39, 40, 65, 68, 83, 87, 13].indexOf(e.keyCode) > -1) {
        ...
        if (e.keyCode === 32) {
            field.pause();
            return;
        } else if (e.keyCode == 13) updateGameArea();
        // if clock isn't already started, start it
        else if (!field.interval) field.startClock();
    }
};
