...
let field = {
    ...
    genSnacks: function (num) {
        for (let i = 0; i < num; i++) {
            field.snacks.push([
                Math.floor((Math.random() * field.canvas.width) / field.delta) *
                    field.delta,
                Math.floor(
                    (Math.random() * field.canvas.height) / field.delta
                ) * field.delta,
    ...
    genObstacles: function (num, size) {
        ...
        let snakeCent = [
            field.canvas.width / 2 - ((field.canvas.width / 2) % field.delta),
            field.canvas.height / 2 - ((field.canvas.height / 2) % field.delta),
        ];
        for (let i = 0; i < num; i++) {
            for (let j = 0; j < size; j++) {
                if (j > 0) {
                    x =
                        field.obstacles[i * size + j - 1][0] +
                        (Math.ceil(Math.random() * 3) - 2) * field.delta;
                    y =
                        field.obstacles[i * size + j - 1][1] +
                        (Math.ceil(Math.random() * 3) - 2) * field.delta;
                    ...
                    x =
                        Math.random() * (field.canvas.width - buffer * 2) +
                        buffer;
                    x = x - (x % field.delta);
                    y =
                        Math.random() * (field.canvas.height - buffer * 2) +
                        buffer;
                    y = y - (y % field.delta);
                    while (!(x != snakeCent[0] && y != snakeCent[1])) {
    ...
    prepGame: function () {
        ...
        this.canvas.width =
            window.innerWidth - field.delta - (window.innerWidth % field.delta);
        this.canvas.height =
            window.innerHeight -
            field.delta -
            (window.innerHeight % field.delta);
        ...
        field.genObstacles(
            (field.canvas.width * field.canvas.height) / 20000,
            8
        );
    ...
    snacks2poison: function () {
        for (let i = 0; i < field.snacks.length; i++) {
            if (
                field.snacks[i][2] + 10 + Math.random() * 10 <
                    new Date().getTime() / 1000 &&
                field.snacks[i][3] == "s"
            ) {
                field.snacks[i][3] = "p";
    ...
    poison2ether: function () {
        for (let i = 0; i < field.snacks.length; i++) {
            if (
                field.snacks[i][2] + 20 + Math.random() * 40 <
                    new Date().getTime() / 1000 &&
                field.snacks[i][3] == "p"
            ) {
...
function snake(x, y) {
    ...
    this.draw = function () {
        ...
        for (let i = 1; i < mySnake.segments.length; i++) {
            field.ctx.fillRect(
                mySnake.segments[i][0],
                mySnake.segments[i][1],
                field.delta,
                field.delta
            );
        }
    ...
    this.updateSnake = function () {
        ...
        for (let i = 0; i < field.snacks.length; i++) {
            if (
                ...
                if (mySnake.segments.length == 1) {
                    ...
                    let len =
                        mySnake.segments.length < 5
                            ? mySnake.segments.length - 1
                            : 5 - 1;
                    for (let j = 0; j < len; j++) {
        ...
        for (let i = 0; i < mySnake.collisions.length; i++) {
            ...
            for (let j = 0; j < mySnake.segments.length; j++) {
                if (
                    mySnake.collisions[i][0] != mySnake.segments[j][0] ||
                    mySnake.collisions[i][1] != mySnake.segments[j][1]
                ) {
                    nonoverlaps += 1;
                }
            }
        ...
        if (mySnake.segments.length - 1 > mySnake.highest)
            mySnake.highest = mySnake.segments.length - 1;
...
function updateGameArea() {
    ...
    // add new snacks
    if (Math.random() > 0.94)
        field.genSnacks((field.canvas.width * field.canvas.height) / 200000);
